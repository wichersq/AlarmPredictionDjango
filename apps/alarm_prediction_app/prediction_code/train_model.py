...
FILE_NAME_WITH_TOTAL_DURATION = 'apps/alarm_prediction_app/prediction_code/model_data.csv'
...
for i in range(60000):
  ...
  samp_x = batch.as_matrix(ml_model.list_samp_x)
  samp_y = batch.as_matrix(['Total_Driving'])
  loss_val, error_val = ml_model.train_iter(samp_x, samp_y)
  if i % 6000 == 0:
    print("loss", loss_val, "\n error_sec", error_val)

    samp_x = test_df.as_matrix(ml_model.list_samp_x)
    samp_y = test_df.as_matrix(['Total_Driving'])
    print("tdrtfhgyjhukji", samp_x[0], samp_y[0])
    loss_val, error_val = ml_model.get_error(samp_x, samp_y)
